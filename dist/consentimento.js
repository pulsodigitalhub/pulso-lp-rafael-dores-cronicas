/*
 * Banner de consentimento de cookies (LGPD + Google Consent Mode v2). Arquivo idêntico em todas as LPs da Pulso; o que muda por site vem dos atributos data-* da própria tag <script>. O consentimento padrão (tudo negado) precisa ser declarado ANTES do GTM, num trecho inline no <head> — este arquivo só mostra o banner e grava a escolha.
 */
(function () {
  'use strict';
  ...
  function createBanner() {
    try {
      var fundo = (script && script.getAttribute('data-fundo')) || '#1f2937';
      var destaque = (script && script.getAttribute('data-destaque')) || '#ffffff';
      var textoDestaque = (script && script.getAttribute('data-texto-destaque')) || '#111827';
      var copy, title, paragraph, link, actions, acceptButton, stored;
      injectStyle();
      banner = document.createElement('section');
      banner.id = 'consent-banner';
      banner.className = 'cc-banner';
      banner.setAttribute('role', 'region');
      banner.setAttribute('aria-labelledby', 'cc-titulo');
      banner.hidden = true;
      banner.style.setProperty('--cc-fundo', fundo);
      banner.style.setProperty('--cc-destaque', destaque);
      banner.style.setProperty('--cc-texto-destaque', textoDestaque);
      ...
      essentialButton.onclick = function () { choose('essential'); };
      acceptButton.onclick = function () { choose('all'); };
      actions.appendChild(essentialButton); actions.appendChild(acceptButton);
      banner.appendChild(copy); banner.appendChild(actions); document.body.appendChild(banner);
  ...
  try {
    ...
    else { document.addEventListener('DOMContentLoaded', createBanner, false); }
  } catch (e) {}

  try { window.reopenConsent = reopenConsent; } catch (e) {}
    ...
    document.addEventListener('click', function (event) {
      try {
        ...
        while (node && node !== document) {
          ...
          if (matches && matches.call(node, '[data-abrir-consentimento], a[href$="#consentimento"]')) {
            event.preventDefault(); reopenConsent(); return;
          }
